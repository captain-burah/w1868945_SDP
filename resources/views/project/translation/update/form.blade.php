<div class="card">
    <div class="card-body">
        <h3 class="mb-1">Translation Segment Update</h3>

        <div class="my-4">
            <form class="contact-form" action="{{ route('project-translation.update', ['project_translation' => $segments->id])  }}" method="POST" enctype="multipart/form-data" id="dropzone"
            >
                @csrf
                @method('PATCH')
                                    ...
                                    >
                                        ...
                                        @if(isset($languages))
                                            @foreach($languages as $data)
                                                @if($data->language_id == $segments->language_id)
                                    ...
                                    >
                                        ...
                                        @if(isset($projects))
                                            @foreach($projects as $data)
                                                @if($data->project_id == $segments->project_id)
                                                    <option selected value="{{$data->id}}">{{ $data->name }}</option>
                                                @else
                                                    <option selected value="{{$data->id}}">{{ $data->name }}</option>
                                                @endif
                                    ...
                                    <div class="w-100 ">
                                        <div class="my-4">
                                            @error('description')
                                                <div class="text-danger text-xs" >{{ $message }}</div>
                                            @enderror
                                            <textarea name="description" id="description" class="w-100">{{ $segments->description }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <input
                                        ...
                                        maxlength="60"
                                        id="meta_description"
                                        value="{{ $segments->meta_description }}"
