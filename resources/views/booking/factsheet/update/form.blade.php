<div class="card">
    <div class="card-body">
        <h3 class="mb-1">Factsheet Segment Update</h3>

        <div class="my-4">
            <form class="contact-form" action="{{ route('project-factsheet.update', ['project_factsheet' => $segments->id])  }}" method="POST" enctype="multipart/form-data" id="dropzone"
            >
                @csrf
                @method('PATCH')
            ...
            @foreach($segment_files as $data)
                <div class="grid grid-rows-3 grid-flow-col gap-4">
                    <div class="inline-block align-baseline my-4 text-center">
                        <form action="{{ route('project-factsheet.destroy', ['project_factsheet' => $data->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                    ...
                    <div class="">
                        <iframe src="{{ url('storage/projects/factsheets/'.$data->project_factsheet->id.'/'.$data->name) }}" width="" height="400">
                        </iframe>
                    </div>
                </div>
